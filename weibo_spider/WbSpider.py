...
class WbSpider(object):
    ...
    def scheduler(self, url, begin_time, end_time, filename, frequency=None, path="./excel_weibo/"):
        ...
        for link in data_generator:
            print(link)
            sys.stdout.write("\r" + self.time + "已下载%.2f%%\tnum_of_rows = %d" % (float(self.page_current/self.page_total * 100), self.num_of_rows))
    ...
    def iter_manager(self, url, begin_time, end_time, frequency=None):
        ...
        for time_str_2 in time_str_generator:
            ...
            for li in li_list:
                page += 1
                a_url = "https://s.weibo.com/weibo?q=" + self.query_str_paras["q"] + "&typeall=1&suball=1&timescope=custom:"+time_str_1+":" + time_str_2 +"&Refer=g&page="+str(page)
                yield a_url
